...
from tools.lane_tools.llamas.llamas_metric import eval_predictions


ANNO_DIR_PATH = './labels/valid/'
...
def generate_culane_lines(out):
    out = out * (LLAMAS_IMG_RES[1] / 640)  # B, 4, 360
    THRESHOLD = 60
    # THRESHOLD = 10
    ...
    for i in range(out.shape[0]):
        x = out[i, :]
        mask = x != LLAMAS_IMG_RES[1]

        # Discard if no line
        if torch.count_nonzero(mask) >= THRESHOLD:
            y = torch.arange(0, len(x)) * (LLAMAS_IMG_RES[0] / 360)
...
def write_culane_data(test_data, save_predict_path):
    ...
    for i, name in enumerate(names):
        ...
        name = name.replace('color_images', 'labels')
        spred_file_path = os.path.join(save_predict_path, os.path.split(name)[0])
        os.makedirs(spred_file_path, exist_ok=True)

        fname = os.path.split(name)[-1].replace('_color_rect.png', '.lines.txt')
        with open(os.path.join(spred_file_path, fname), 'w') as fp:
            for lane in lanes:
...
def single_gpu_test(model, data_loader, anno_dir, out_dir=None, training_test=False):
    ...
    eval_result = eval_predictions(os.path.join(save_predict_path, './labels/valid/'),
                                   anno_dir=anno_dir,
                                   unofficial=False,
                                   sequential=False)
...
def multi_gpu_test(model, data_loader, anno_dir, out_dir, training_test=False):
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            start_events.append(tic)
            end_events.append(toc)
    ...
    total_run_time = sum([s.elapsed_time(e) for s, e in zip(start_events, end_events)])
    save_predict_path = os.path.join(out_dir, PRED_SAVE_DIR)
    os.makedirs(save_predict_path, exist_ok=True)
    ...
    if rank == 0:
        eval_result = eval_predictions(pred_dir=os.path.join(save_predict_path, './labels/valid/'),
                                       anno_dir=anno_dir,
                                       unofficial=False,
                                       sequential=False)
        eval_result['FPS'] = 1000. / (total_run_time / total_num)
...
def main():
    ...
    ann_dir = os.path.join(cfg.data.test.data_root, ANNO_DIR_PATH)
    if not distributed:
        model = MMDataParallel(model, device_ids=[0])
        single_gpu_test(model, data_loader, ann_dir, cfg.work_dir, training_test=False)
    else:
        model = MMDistributedDataParallel(
            model.cuda(),
            device_ids=[torch.cuda.current_device()],
            broadcast_buffers=False)
        multi_gpu_test(model, data_loader, ann_dir, cfg.work_dir, training_test=False)
